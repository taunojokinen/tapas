...
const RenderAIProposals: React.FC<{
  ...
  const [valueProposalUpdate, setValueProposalUpdate] = useState<Proposal[]>(
    []
  ...
  async function* fetchValueProposals(
    role: string,
    prompt: string
      ...
      for (const role of rolesForAI) {
        ...
        for await (const proposal of fetchValueProposals(role, aiPrompt)) {
          ...
          if (
            ...
            console.error(
              `Invalid proposal.values for role: ${role}`,
              proposal
            );
          }
        }
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-4">
        Ehdotuksia arvoista, joilla voit korvata nykyisiä arvoja
      </h2>
      {loading && (
      ...
      {valueProposal.map((proposal, index) => (
        <div key={index} className="mb-6">
          <h2 className="text-xl font-bold mb-4">
            {proposal.role} ehdottaa uusia arvoja
          </h2>
          {proposal.values &&
