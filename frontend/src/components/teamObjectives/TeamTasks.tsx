...
import { patchTeamObjectiveData } from "./TeamObjectiveFunctions";
...
const TeamTasks: React.FC<TeamTasksProps> = ({ tasks, setTasks, teamName }) => {
  ...
  const handleAddTask = async () => {
    ...
    await patchTeamObjectiveData(teamName, payload);
  };
  ...
  const handleDeleteTask = async (index: number) => {
    ...
    // Save changes
    await patchTeamObjectiveData(teamName, { tasks: updatedTasks });
  };
  ...
  const handleMoveTask = async (index: number, direction: "up" | "down") => {
    ...
    if (swapIndex >= 0 && swapIndex < tasks.length) {
      ...
      // Save changes
      await patchTeamObjectiveData(teamName, { tasks: newTasks });
    }
  };
