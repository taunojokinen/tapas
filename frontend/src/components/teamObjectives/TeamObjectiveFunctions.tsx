...
export async function fetchCompanyObjectives(
  query: string
  ...
  try {
    ...
    if (query) {
      ...
      filtered = objectivesList.filter(
        (obj: CompanyObjectives) =>
          obj.avainstrategiat.some((strategia) =>
            strategia.tavoite.toLowerCase().includes(lowerQuery)
          ) ||
          obj.avainstrategiat.some((strategia) =>
            strategia.toimenpide.toLowerCase().includes(lowerQuery)
          ) ||
          obj.avainstrategiat.some((strategia) =>
            strategia.seuranta.toLowerCase().includes(lowerQuery)
          )
      );
    }
    // Map CompanyObjectives to TeamObjective[] (do NOT set _id, let MongoDB generate it)
    const teamObjectives: TeamObjective[] = filtered.flatMap(
      (obj: CompanyObjectives) =>
...
export async function putTeamObjectiveData(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to update team objective. Status: ${response.status}`
      );
    }
...
export async function putTeamObjectivesArray(
  teamId: string,
  objectives: any[]
): Promise<void> {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to update team objectives array. Status: ${response.status}`
      );
    }
